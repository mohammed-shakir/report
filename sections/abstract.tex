\begin{abstract}
Operational web mapping stacks commonly pair GeoServer with PostGIS, yet repeated vector queries over urban hotspots recompute overlapping work, raising latency and database load. This thesis studies an adaptive cache that partitions request footprints into H3 hexagonal cells, stores per-cell results in Redis, and maintains freshness via time-to-live (\ac{ttl}) plus Kafka-driven invalidation. A Go middleware is implemented that maps \ac{bbox}/polygon filters to H3 cells, composes per-cell payloads on cache hits, deduplicates features, and reapplies the exact spatial and attribute filter.
...
It is concluded that H3-guided, per-cell caching complements \ac{gist}-indexed PostGIS for dynamic vector workloads. Guidance is to start at $r=7$, monitor keys-per-request and merge time, size Redis to the working set, and pair per-layer TTLs with event-driven invalidation to bound staleness while maximizing reuse.
\end{abstract}
