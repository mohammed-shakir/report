...
Request footprints are deterministically converted into H3 cell sets. A rectangular BBOX is treated as a four-vertex polygon, while custom polygons are accepted as-is. The conversion uses H3's region functions: given a polygon and a target resolution, return the set of cell IDs whose centroids lie inside the polygon \cite{H3_API_Regions}. This centroid-containment rule is explicit in both the H3 reference and in SQL integrations. The inverse operation reconstructs polygon outlines from a set of cells, which we use for debugging and visualization \cite{H3_Py_Polygon_Tutorial}.

Boundary effects must be handled carefully. Because containment is centroid-based, a thin polygon may exclude edge-touching cells whose centers lie just outside; on the other hand, low resolution cells can over-cover the footprint. We address these issues by: choosing a resolution appropriate with the query's spatial granularity, optionally expanding the boundary by a 1-ring and clipping results to the original geometry, and always reapplying the original attribute/spatial filter when combining cached cells to avoid false positives. For multipolygons and holes, we pass the outer ring and interior rings to the region function so that coverage excludes holes and unconnected islands are handled correctly. Neighboring cells are included only when an explicit buffer around the footprint is needed for later rendering or prefetch operations \cite{H3_API_Traversal}.
